...
Mesh::Mesh(std::string const &filename) : Mesh() {
  ...
  aiScene const *scene =
      loader.ReadFile(PROJECT_SOURCE_DIR "/Glitter/Models/" + filename,
...
void Mesh::draw(GLuint shader) {
  ...
  for (auto &i : mSubMeshes)
    i->draw(shader);
  for (auto &i : mTextures) { // Set Correct Uniform Names Using Texture Type
                              // (Omit ID for 0th Texture)
    std::string uniform = i.second;
    if (i.second == "diffuse")
      uniform += (diffuse++ > 0) ? std::to_string(diffuse) : "";
    else if (i.second == "specular")
      uniform += (specular++ > 0) ? std::to_string(specular) : "";
...
void Mesh::parse(std::string const &path, aiNode const *node,
                 aiScene const *scene) {
...
void Mesh::parse(std::string const &path, aiMesh const *mesh,
                 aiScene const *scene) {
  ...
  auto diffuse = process(path, scene->mMaterials[mesh->mMaterialIndex],
                         aiTextureType_DIFFUSE);
  auto specular = process(path, scene->mMaterials[mesh->mMaterialIndex],
